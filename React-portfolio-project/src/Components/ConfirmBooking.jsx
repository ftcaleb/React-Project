...
function ConfirmBooking() {
  ...
  const handleSaveBooking = async () => {
    ...
    try {
      const response = await fetch(
        "https://react-project-blw8.onrender.com/api/bookings",
        {
          method: "POST",
      ...
      // Redirect to success page or homepage after saving
  ...
  return (
    <div className="flex flex-col justify-center border bg-white text-black rounded shadow-sm m-10 p-5 w-100 h-100">
      <div className="font-large bg-black text-white p-4 rounded">
        Booking Confirmed!
      </div>

      <div className="flex justify-between items-center text-black p-2 rounded-2xl font-medium">
        Date: {dateString}
      </div>
      <div className="flex justify-between items-center text-black p-2 rounded-2xl font-medium">
        Service: {service || "No service selected"}
      </div>
      <div className="flex justify-between items-center text-black p-2 rounded-2xl font-medium">
        Time: {time || "No time selected"}
      </div>
      <div className="flex justify-between items-center text-black p-2 rounded-2xl font-medium">
        Duration: {duration || "No duration selected"}
      </div>
      <div className="flex justify-between items-center text-black p-2 rounded-2xl font-medium">
